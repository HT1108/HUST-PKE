...
void load_user_program(process *proc) {
  ...
  proc->trapframe = (trapframe*)(USER_TRAP_FRAME + hartid * 0x00080000);
  memset(proc->trapframe, 0, sizeof(trapframe));
  // USER_KSTACK is also a physical address defined in kernel/config.h
  proc->kstack = (USER_KSTACK + hartid * 0x00080000);
  proc->trapframe->regs.sp = USER_STACK + hartid * 0x00080000;
